...
export class ScheduleService {
      ...
      if (error.code === "23505") {
        if (error.constraint === "unique_schedule_day_classroom_time") {
          throw new HTTPException(409, {
            message:
              "One or more courses have schedule conflicts (same time and place)",
          });
        }
        if (
          error.constraint === "courses_code_key" ||
          error.constraint === "courses_code_unique"
        ) {
          throw new HTTPException(409, {
        ...
        throw new HTTPException(409, {
          message: "Schedule or course data conflict",
        });
      }
  ...
  async updateSchedule(
    ...
    if (
      ...
      (!courseDatas || courseDatas.length === 0)
    ) {
    ...
    try {
        ...
        if (courseDatas && courseDatas.length > 0) {
          ...
          for (const courseData of courseDatas) {
            if (courseData.id) {
              ...
              await tx
                .insert(courses)
                .values({ ...courseData, scheduleId: id } as any);
            }
          }
        }
      ...
      if (error instanceof DatabaseError && error.code === "23505") {
        throw new HTTPException(409, {
          message: "Schedule or course conflict",
        });
      }
  ...
  async deleteSchedule(id: number): Promise<void> {
    try {
      ...
      if (error instanceof DatabaseError && error.code === "23503") {
        throw new HTTPException(409, {
          message:
            "Cannot delete schedule because it is linked to existing attendance records or other data.",
        });
      }
      throw error;
    }
  ...
  async getDailySchedule(date: string, facultyId?: number) {
      ...
      .map((c) => {
        ...
        if (override) {
          return {
            ...
            classroom:
              override.replacementClassroom || (c.schedule as any).classroom,
            isOverridden: true,
    ...
    if (facultyId) {
      schedule = schedule.filter(
        (c) => (c.schedule as any).facultyId === facultyId,
      );
    }
